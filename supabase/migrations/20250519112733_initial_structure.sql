...
CREATE OR REPLACE FUNCTION public.increment_entries()
 RETURNS trigger
 LANGUAGE plpgsql
AS $function$BEGIN
  update public.stats
...
create policy "Enable update based on user id"
on "public"."entries"
as permissive
for update
to authenticated
...
CREATE TRIGGER "INCREMENT_ENTRIES" AFTER INSERT ON public.entries FOR EACH STATEMENT EXECUTE FUNCTION increment_entries();
